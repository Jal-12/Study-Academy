...
if (mysqli_num_rows($SubSqlResult)==0) {
    ...
    while ($subRow=mysqli_fetch_assoc($SubSqlResult)) {
       if ($subRow['sub_id'] == $sub_id) {
            echo '<a class="btn btn-dark  m-2 my-4" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$subRow['sub_id'].'">' . sub_id($subRow['sub_id']) . '</a>';
        } else {
            echo '<a class="btn btn-light  m-2 my-4" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$subRow['sub_id'].'">' . sub_id($subRow['sub_id']) . '</a>';
        }
    }
}
echo'</center>';
...
?>
    ...
    <center>
                    ...
                    if (isset($unitSQL)) {
                        ...
                        <a class="shadow" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$sub_id.'&unit_id='.$unitRow['unit_id'].'"><div class="col-sm-11 btn btn-sm btn-dark  my-3">'.$unitArr["unit_number"].'.&nbsp;'.$unitArr["unit_name"].'</div></a> ';
                            } else {
                                echo '<a class="shadow-sm" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$sub_id.'&unit_id='.$unitRow['unit_id'].'"><div class="col-sm-11 btn btn-sm btn-light my-3">'.$unitArr["unit_number"].'.&nbsp;'.$unitArr["unit_name"].'</div></a>';
                            }
...
?>    
<div class="col-sm-6 my-4 col-lg-4 p-4">
    <div class="">
...
if ($records>0) {
            ...
            for ($i=1; $i<=$total_page; $i++) {
                ...
                if (isset($_GET['unit_id'])&& $_GET['unit_id'] != '') {
                    $PageNo='<li class="page-item '.$active.'"><a class="page-link" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$sub_id.'&unit_id='.$unit_id.'&page='.$i.'">'.$i.'</a></li>';
                    
                }else{
                    
                    $PageNo='<li class="page-item '.$active.'"><a class="page-link" href="view_lectures_other_sem.php?semester='.$semester.'&sub_id='.$sub_id.'&page='.$i.'">'.$i.'</a></li>';
                }
