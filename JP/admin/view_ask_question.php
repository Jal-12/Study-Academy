                    ...
                    if (isset($subSql)) {
                        ...
                        while ($subRow = mysqli_fetch_assoc($subResult)) {
                            ...
                            if ($subRow['sub_id'] == $sub_id1) {
                                echo '<a class="btn btn-warning  m-2 my-4" href="view_ask_question.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id=' . $subRow['sub_id'] . '">' . sub_id($subRow['sub_id']) . '</a>';
                            } else {
                                echo '<a class="btn btn-primary  m-2 my-4" href="view_ask_question.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id=' . $subRow['sub_id'] . '">' . sub_id($subRow['sub_id']) . '</a>';
                            }                            
                        }
                    }
                    ?>
					...
					while ($unitRow = mysqli_fetch_assoc($unitResult)) {
						$unitArr=unit_id($unitRow['unit_id']);
							if ($unitRow['unit_id'] == $unit_id) {
								echo '<a class="btn btn-sm btn-warning  m-2 my-2" href="view_ask_question.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id='.$sub_id1.'&unit_id='.$unitRow['unit_id'].'">'.$unitArr["unit_number"].'.&nbsp;'.$unitArr["unit_name"].'</a>';
							} else {
								echo '<a class="btn btn-sm btn-primary  m-2 my-2" href="view_ask_question.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id='.$sub_id1.'&unit_id='.$unitRow['unit_id'].'">'.$unitArr["unit_number"].'.&nbsp;'.$unitArr["unit_name"].'</a>';
							}					
						}
					}
				?>
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if($row["update_time"]==NULL){
                ...
                $que_time=$row["update_time"];
                      ...
                      ?>               
                            <tr>
                                <td style="text-align: center"><h6><?php echo "<b>".$No."</b>"; ?></h6></td>
                                <?php
					            	if (!isset($subSql))
						            {
                                ...
                                <td>
									<a href="ask_answer.php?que_id=<?php echo $row['que_id']; ?>" class="btn  btn-primary" href="">Answer</a>
								</td>
								<td >
