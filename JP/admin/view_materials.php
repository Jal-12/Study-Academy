...
<div class="card">
				...
				$subResult = mysqli_query($con, $subSql);
				 while ($subRow = mysqli_fetch_assoc($subResult)) {
					...
					if ($subRow['sub_id'] == $sub_id1) {
						echo '<a class="btn btn-warning  m-2 my-4" href="view_materials.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id=' . $subRow['sub_id'] . '">' . sub_id($subRow['sub_id']) . '</a>';
					} else {
						echo '<a class="btn btn-primary  m-2 my-4" href="view_materials.php?d_id=' . $d_id . '&sem_id=' . $sem_id . '&sub_id=' . $subRow['sub_id'] . '">' . sub_id($subRow['sub_id']) . '</a>';
						}
					}
				}
			?>
						...
						<table class="table thover ts table-bordered">
							<thead class="table-primary">
								<tr class="">
									...
									if (!isset($subSql))
									 {
										echo "<th width='2%' style='text-align: center'>Semester</th>";
									}
									if ($flag==false) {
					    			...
					    			<th width="6%" colspan="2" style="text-align: center">Action</th>
								</tr>
							</thead>
							<tbody>
	<?php
	...
	?>
				<tr>
					...
					<embed src="<?php echo $row['file'] ?>" width="350px" height="120px" ></embed>
					<a href="<?php echo $row['file']; ?>" target="_blank"><h6>View File</h6></a></td>
					<td style="text-align: center">
						<a href="edit_materials.php?material_id=<?php echo $row['material_id']; ?>" class="btn btn-primary" >Update</a>
					</td>
					<td style="text-align: center">
