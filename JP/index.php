<!DOCTYPE html>

<html dir="ltr">
    <head>
        <meta charset="utf-8">
         <title> Home </title>
         <link rel="stylesheet" href="css/Index.css">
         <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.14.0/css/all.min.css">
    </head>
    <body>
         <div class="header">
          
             <div class="header-menu">
             <i class="fas-fa-bars"></i>
              <ul>
               <li><a href="student/Login.php"><i class="fas fa-users"><p>Student</p></i></a></li>
               <li><a href="faculty/Verification.php"><i class="fas fa-user-graduate"><p>Faculty</p></i></a></li>
               <li><a href="admin/Verification.php"><i class="fas fa-user-lock"><p>Admin</p></i></a></li>
               <li><a href="#"><i class="fas fa-exclamation-circle"><p>About us</p></i></a></li>
               <li><a href="#"><i class="fas fa-question-circle"><p>Help</p></i></a></li>
              </ul>
             </div>
             </div>
          <div class="main-center">
          <div class="card center">
            <p class="heading">Welcome
           <p class="title"> Study Academy </p>  
            </p>
           <p class="text"> 
              We design and develop experiences that make
             people's lives simple.
            </p>
          </div>
          </div> 
    </body>   
</html> 